...
<nav class="navbar navbar-expand-lg navbar-dark fixed-top">
    <div class="container-fluid">
        <a class="navbar-brand d-flex align-items-center" href="{{ route('responder.dashboard') }}">
        ...
        <div class="collapse navbar-collapse" id="responderNavbar">
            <ul class="navbar-nav center-nav">
                <li class="nav-item">
                    <a href="{{ route('responder.dashboard') }}" class="nav-link">
                        <i class="bi bi-speedometer2"></i> Dashboard
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('responder.assigned-incidents') }}" class="nav-link">
                        <i class="bi bi-clipboard-check"></i> Assigned Incidents
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('responder.completed') }}" class="nav-link">
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle d-flex align-items-center" href="#" id="userDropdown"
                       role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        ...
                        <span>{{ auth()->user()->name ?? 'Responder' }}</span>
                    </a>
                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="userDropdown">
                        <li>
                            <a class="dropdown-item" href="{{ route('responder.edit', ['id' => Auth::id()]) }}">
...
async function checkResponderNotifications() {
    try {
        const res = await fetch(`${BASE_URL}{{ route('responder.notifications.fetch', [], false) }}`);
