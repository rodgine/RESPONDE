...
<div class="content-card">
    <div class="container py-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap">
            ...
            <form id="backupForm" action="{{ route('admin.backup.run') }}" method="POST" style="display:none;">
                @csrf
            ...
            <div class="card-body table-responsive">
                ...
                <table class="table align-middle table-hover mb-0" id="backupTable">
                    ...
                    @forelse($backups as $index => $b)
                        <tr>
                            <td class="fw-semibold">{{ $b->file_name }}</td>
                            <td>{{ number_format($b->file_size / 1024, 2) }} KB</td>
                            <td>{{ $b->created_at->format('M d, Y H:i') }}</td>
                            <td>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    <a href="{{ route('admin.backup.download', $b->id) }}" 
                                    ...
                                    <button class="btn btn-sm btn-outline-primary me-1 restoreBtn" 
                                            data-id="{{ $b->id }}" data-bs-toggle="tooltip" title="Restore">
                                        <i class="bi bi-arrow-counterclockwise"></i>
                                    </button>

                                    <button class="btn btn-sm btn-outline-danger deleteBtn" 
                                            data-id="{{ $b->id }}" data-bs-toggle="tooltip" title="Delete">
                                        <i class="bi bi-trash"></i>
                                    </button>
                                </div>

                                <!-- Hidden Forms -->
                                <form id="restoreForm-{{ $b->id }}" method="POST" 
                                      action="{{ route('admin.backup.restore', $b->id) }}" style="display:none;">
                                    @csrf
                                    <input type="hidden" name="confirm_password" id="restore_confirm_password_{{ $b->id }}">
                                </form>

                                <form id="deleteForm-{{ $b->id }}" method="POST" 
                                      action="{{ route('admin.backup.delete', $b->id) }}" style="display:none;">
                                    @csrf
                                    @method('DELETE')
                                    <input type="hidden" name="confirm_password" id="delete_confirm_password_{{ $b->id }}">
                                </form>
                            </td>
                        </tr>
                    @empty
...
{{-- Spinner overlay (shows during long actions) --}}
...
@if(session('success'))
    Toast.fire({ icon: 'success', title: @json(session('success')) }).then(()=> {
        hideOverlay();
...
@if(session('error'))
    Swal.fire({ icon: 'error', title: 'Failed', text: @json(session('error')) }).then(()=> {
        hideOverlay();
    });
@endif
</script>
